...
run_on_server "
    echo '[SSH LOG] Starting Step 2 model setup at \$(date)'
    echo '[SSH LOG] Current working directory: \$(pwd)'
    echo '[SSH LOG] Available disk space:'
    df -h /opt/riva || echo '[SSH LOG] /opt/riva not accessible'

    cd /opt/riva
    echo '[SSH LOG] Changed to /opt/riva directory'
    echo '[SSH LOG] Contents of /opt/riva:'
    ls -la
    ...
    if [ -d deployed_models ] && find deployed_models -name config.pbtxt 2>/dev/null | grep -q .; then
        ...
        echo '[SSH LOG] Checking cache directory and S3 access'
        echo '[SSH LOG] Cache contents:'
        ...
        if [ $BUILD_EXIT_CODE -eq 0 ]; then
            ...
            echo '[SSH LOG] Checking for riva_deploy.sh'
            pwd > /tmp/deploy_dir.txt
            if [ ! -f "riva_deploy.sh" ]; then
                echo "❌ riva_deploy.sh not found in \$(cat /tmp/deploy_dir.txt)"
                echo '[SSH LOG] Available files:'
                ls -la *.sh 2>/dev/null || echo 'No .sh files found'
                exit 1
            fi
            echo '[SSH LOG] riva_deploy.sh found successfully'
            ...
            # Verify deployment
            find ../deployed_models -name config.pbtxt | wc -l > /tmp/model_count.txt
            DEPLOYED_MODELS=\$(cat /tmp/model_count.txt)
            ...
            echo "   • Working directory: \$(cat /tmp/current_dir.txt)"
