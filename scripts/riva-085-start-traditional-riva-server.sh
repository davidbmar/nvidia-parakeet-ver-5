...
# RIVA-044: Start Riva Server with Deployed Models
# This script starts NVIDIA Riva ASR server using deployed Triton models
#
# Prerequisites:
# - riva-043 completed (models deployed to Triton format)
# - /opt/riva/deployed_models/asr/ contains model directories
#
# Next script: riva-045-deploy-websocket-app.sh
...
echo -e "${BLUE}🚀 RIVA-044: Start Riva Server with Deployed Models${NC}"
echo "================================================================"
...
docker run -d \\
    ...
    -v /opt/riva/simple_models:/data/models \\
...
run_on_server "
    echo 'Checking .riva models in simplified structure...'
    if [ -d /opt/riva/simple_models ] && find /opt/riva/simple_models -name '*.riva' | head -3; then
        echo '✅ .riva models ready for deployment'
        echo \"Total .riva files: \$(find /opt/riva/simple_models -name '*.riva' | wc -l)\"
        echo 'Model files:'
        find /opt/riva/simple_models -name '*.riva' | head -3
    else
        echo '❌ No .riva models found in simplified structure'
        echo 'Creating simplified structure from downloaded models...'
        if find /opt/riva/models -name '*.riva' | head -1; then
            sudo mkdir -p /opt/riva/simple_models
            sudo find /opt/riva/models -name '*.riva' -exec cp {} /opt/riva/simple_models/ \\;
            sudo chown -R ubuntu:ubuntu /opt/riva/simple_models
            echo 'Simple structure created'
        else
            echo 'Please run: ./scripts/riva-042-download-models.sh first'
            exit 1
        fi
    fi
...
for i in {1..12}; do
    ...
    HEALTH_STATUS=$(run_on_server "curl -s -o /dev/null -w '%{http_code}' http://localhost:$RIVA_HTTP_PORT/v2/health/ready || echo '000'" "")
