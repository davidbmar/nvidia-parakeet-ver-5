#!/bin/bash
#
# RIVA-062: Deploy Single NIM Model (Streaming + Punctuation)
# T4-optimized deployment with streaming low-latency model + punctuation
# Memory efficient: ~8-10GB GPU usage, leaves 5-7GB headroom
#
...
print_script_header "062" "Deploy Single NIM Model (Streaming + Punctuation)" "T4-optimized single-model deployment"

# Configuration for T4 deployment
CONTAINER_IMAGE="nvcr.io/nim/nvidia/parakeet-ctc-riva-1-1b:1.0.0"
CONTAINER_NAME="parakeet-nim-streaming"
MODEL_PROFILE="streaming_low_latency_with_punctuation"
...
print_step_header "2" "Configure Single Model Environment"

echo "   🔧 Setting up T4-optimized NIM configuration..."
...
run_remote "
    ...
    # Create model profile configuration
    cat > /opt/nim-config/streaming_profile.json << 'EOF'
{
  \"model_name\": \"parakeet-ctc-1.1b\",
  \"model_profile\": \"streaming_low_latency_with_punctuation\",
  \"models_to_load\": [
    {
      \"name\": \"asr_parakeet_ctc_1.1b_streaming\",
      \"type\": \"streaming\",
      \"priority\": \"high\",
      \"chunk_size_ms\": 160,
      \"max_batch_size\": 4,
...
EOF
    ...
    echo '✅ Model profile configuration created'
"

print_step_header "3" "Start Single Model NIM Container"

echo "   🚀 Starting T4-optimized NIM container..."
run_remote "
    echo 'Starting single-model NIM container...'
    ...
    # Start container with single model configuration
    docker run -d \\
        --name ${CONTAINER_NAME} \\
    ...
    echo '✅ Single-model NIM container started'
    echo 'Container status:'
    docker ps | grep ${CONTAINER_NAME} || echo 'Container starting...'
"

print_step_header "4" "Monitor Single Model Loading"

echo "   ⏳ Monitoring single-model startup (expected 3-5 minutes)..."
run_remote "
    echo 'Waiting for streaming model + punctuation to load...'
...
print_step_header "5" "Test Single Model Health"

echo "   🏥 Testing single-model endpoints..."
run_remote "
...
echo "   📝 Updating environment with single-model configuration..."
update_or_append_env "NIM_CONTAINER_DEPLOYED" "single_model"
update_or_append_env "NIM_DEPLOYMENT_TYPE" "streaming_with_punctuation"
update_or_append_env "NIM_MODEL_PROFILE" "streaming_low_latency_with_punctuation"
update_or_append_env "NIM_CONTAINER_NAME" "$CONTAINER_NAME"
update_or_append_env "NIM_CHUNK_SIZE_MS" "160"
...
echo ""
echo "💡 Quick model swap will be available after running script 064"
echo ""